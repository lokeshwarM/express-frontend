...
type Section = "dashboard" | "listeners" | "users" | "flags" | "reviews" | "transactions" | "sessions";
...
const txColors: Record<string, string> = {
  RECHARGE: "#22c55e",
  SESSION_DEBIT: "#ef4444",
  LISTENER_CREDIT: "#22c55e",
...
const txLabels: Record<string, string> = {
  RECHARGE: "Recharge",
  SESSION_DEBIT: "Call Deduction",
  LISTENER_CREDIT: "Listener Earning",
...
export default function AdminPanel() {
    ...
    try {
      switch (sec) {
        ...
        case "users":
          ...
          break;
        case "flags":
          setFlags(await api.adminGetFlags());
          break;
        case "reviews":
          setReviews(await api.adminGetReviews());
          break;
        case "transactions":
          setTransactions(await api.adminGetTransactions("ALL"));
          break;
        case "sessions":
          setSessions(await api.adminGetSessions());
          break;
      }
    } catch (e) {
      console.error(e);
  ...
  const handleBlacklist = async (listenerId: string, blacklist: boolean) => {
    try {
      ...
      showAction(e instanceof Error ? e.message : "Action failed");
    }
  };
    ...
    <div style={s}>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 32 }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, margin: "0 0 4px", color: "#fff" }}>
            Express
          </h2>
          <p style={{ fontSize: 11, color: "#444", margin: 0, textTransform: "uppercase", letterSpacing: 1 }}>
            Admin Panel
          </p>
        </div>
            ...
            {section === "dashboard" && stats && (
              <>
                ...
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
                  ...
                  {card("Total Sessions", stats.totalSessions, "#fff")}
                </div>
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  {card("Total Revenue", `₹${stats.totalRevenue.toFixed(2)}`, "#22c55e", "Platform commission")}
                  {card("Total Recharges", `₹${stats.totalRecharges.toFixed(2)}`, "#3b82f6", "User recharges")}
                    ...
                    <tbody>
                      {filteredListeners.map((l) => (
                        <tr key={l.id} style={{ background: l.blacklisted ? "#ef444408" : "transparent" }}>
                          ...
                          <td style={tdStyle}>
                            <span style={{ color: "#f59e0b" }}>★</span> {l.rating.toFixed(1)}
                          </td>
                          ...
                          <td style={tdStyle}>
                            {l.blacklisted ? (
                              <span style={{ color: "#ef4444", fontSize: 12, fontWeight: 600 }}>⛔ Blacklisted</span>
                            ) : l.available ? (
                          ...
                          <td style={tdStyle}>
                            <div style={{ display: "flex", gap: 6 }}>
                              {l.blacklisted ? (
                                <button
                                  onClick={() => handleBlacklist(l.id, false)}
                                  style={{
                                    padding: "5px 10px", background: "#22c55e22",
                                ...
                                <button
                                  onClick={() => handleBlacklist(l.id, true)}
                                  style={{
                                    padding: "5px 10px", background: "#ef444422",
                              ...
                              {l.flagCount > 0 && (
                                <button
                                  onClick={() => handleResetFlags(l.id)}
                                  style={{
                                    padding: "5px 10px", background: "#f59e0b22",
            ...
            {section === "users" && (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                  ...
                  <input style={inputStyle} placeholder="Search by email..."
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      {flags.map((f) => (
                        <tr key={f.id}>
                          <td style={tdStyle}>
                            <div style={{ fontWeight: 600, fontSize: 13 }}>{f.listenerEmail}</div>
                          </td>
                          <td style={tdStyle}>
                            <span style={{
                          ...
                          <td style={{ ...tdStyle, color: "#555", fontSize: 12 }}>
            ...
            {section === "reviews" && (
              <>
                <div style={{ marginBottom: 24 }}>
                  ...
                  <p style={{ color: "#555", fontSize: 14, margin: 0 }}>{reviews.length} total reviews</p>
                </div>
                    ...
                    <div key={r.id} style={{
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div>
                          ...
                          {r.comment && (
                            <p style={{ color: "#bbb", fontSize: 13, margin: "0 0 8px" }}>{r.comment}</p>
                          )}
                        ...
                        <p style={{ color: "#444", fontSize: 11, margin: 0, whiteSpace: "nowrap" }}>
                    ...
                    <tbody>
                      {transactions.map((t) => (
                        <tr key={t.id}>
                          <td style={tdStyle}>
                            <span style={{
                              color: txColors[t.type] || "#888",
                              fontSize: 12, fontWeight: 600,
                            }}>{txLabels[t.type] || t.type}</span>
                          </td>
                    ...
                    <tbody>
                      {sessions.map((s) => (
                        <tr key={s.id}>
                          ...
                          <td style={tdStyle}>
                            {s.type === "VOICE" ? "🎙 Voice" : "🎥 Video"}
                          </td>
                          <td style={tdStyle}>{formatDuration(s.durationSeconds)}</td>
                          <td style={tdStyle}>
                            <span style={{
                              fontSize: 11, fontWeight: 600, padding: "3px 8px", borderRadius: 4,
                              background: s.status === "ENDED" ? "#22c55e20" :
                                          s.status === "STARTED" ? "#f59e0b20" : "#2a2a2a",
                              color: s.status === "ENDED" ? "#22c55e" :
                                     s.status === "STARTED" ? "#f59e0b" : "#555",
                            }}>{s.status}</span>
                          </td>
                          <td style={{ ...tdStyle, color: "#555", fontSize: 12 }}>
